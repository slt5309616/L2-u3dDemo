Shader "Tasharen/Water_MC"
{
	Properties
	{
		...
		_Foamness ("Foamness", Range(0.01, 1.0)) = 0.35
		_Transparent ("Transparent", Range(0.01, 1.0)) = 0.1
		_FlowSpeed ("Speed", Range(-2.0, 2.0)) = 0.5
		_InvRanges ("Inverse Alpha(>0), Depth(<0) and Color ranges(0~1)", Vector) = (1.0, 0.17, 0.17, 1.0)
	}
	...
	SubShader
	{
		...
		{
			o.worldPos = float3(v.texcoord.x,0,v.texcoord.y);
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			float offset = _Time.x  * 0.5 * _FlowSpeed;
			half2 tiling = IN.worldPos.xz * _Tiling;


			//half4 pixel1 = tex2D(_WaterTex, tiling + offset);
			//half4 pixel2 = tex2D(_WaterTex, half2(-tiling.y, tiling.x) - offset);
			//half3 normal1 = UnpackNormal(pixel1);
			//half3 normal2 = UnpackNormal(pixel2);
			//half4 nmap = half4((normal1 + normal2) * 0.5,(pixel1.a + pixel2.a)*0.5);
			...
			half foam = nmap.a * (1.0 - abs(col.a * 2.0 - 1.0)) * _Foamness;
			...
			o.Albedo = lerp(refraction, reflection, _Transparent*fresnel) + foam;
			//o.Albedo = refraction * _Transparent + reflection*_ReflectionTint  + foam;
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			// Calculate the initial material color
			o.Albedo = lerp(refraction, reflection, fresnel*_Transparent) + foam;
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			// Calculate the initial material color
			o.Albedo = lerp(reflection, refraction, fresnel * _Transparent);
